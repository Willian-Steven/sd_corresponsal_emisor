 ...
 * @see co.edu.campusucc.sd.modelo.Empleados
 * @author Hibernate Tools
 */
public class EmpleadosHome {

	private static final Logger logger = Logger.getLogger(EmpleadosHome.class.getName());
	...
	public Empleados findById(java.lang.String id) {
		...
		try {
			Empleados instance = (Empleados) sessionFactory.getCurrentSession()
					.get("co.edu.campusucc.sd.modelo.Empleados", id);
			if (instance == null) {
	...
	public List findByExample(Empleados instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("co.edu.campusucc.sd.modelo.Empleados")
